...
const createClothMaterial = () => {
    return new THREE.ShaderMaterial({
        transparent: true,
        depthWrite: false, // Ensure transparent layering works
        uniforms: {
            map: { value: null },
            opacity: { value: 1.0 },
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full bg-gray-100 p-4 rounded-lg">
            <p className="text-gray-600 mb-4 font-mono text-sm">
                WebGL not supported. Rendering simple grid.
            </p>
            <div className="grid grid-cols-3 gap-4">
                {normalizedImages.map((img, i) => (
                    <img
                        ...
                        src={img.src}
                        alt={img.alt}
                        className="w-full h-16 object-contain filter grayscale opacity-70"
                    />
                ))}
            </div>
        </div>
...
export default function InfiniteGallery({
    ...
    fadeSettings = {
        ...
        fadeOut: { start: 0.5, end: 0.53 }, // Adjusted to fade out sooner so they don't clip camera
    },
    blurSettings = {
        blurIn: { start: 0.0, end: 0.1 },
        blurOut: { start: 0.4, end: 0.53 },
        maxBlur: 8.0,
    },
